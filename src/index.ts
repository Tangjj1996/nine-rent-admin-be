...
async function updateOrInsertUserData(userData: any) {
  ...
  if (existingHouse.length > 0) {
    // 存在则更新
    await db.update(house).set(userData).where(eq(house.id, id)).execute();
  } else {
    // 不存在则新建
    await db.insert(house).values(userData).execute();
  }
}

const modifySchemaHandle = factory.createHandlers(async (c) => {
  const userData = await c.req.json();
  const { success, error, data } = userDataSchema.safeParse(userData);
  if (success) {
    const composeData: Omit<typeof data, "tags" | "swiper"> & {
      tags: string;
      swiper: string;
    } = {
      tags: "",
      swiper: "",
      ...omit(data, ["tags", "swiper"]),
    };
    composeData.tags = data.tags.join(",");
    composeData.swiper = data.swiper.join(",");
    await updateOrInsertUserData(composeData);
